...
from eatb.packaging import create_package
from eatb.csip_validation import ValidationResult, XmlValidation
...
from eatb.utils.XmlHelper import XSI_NS, q
...
def create_aip(package_dir, identifier, package_name, identifier_map=None, generate_premis=True,
    ...
    if identifier_map:
        ...
        # print sip_mets_path
        sip_parse = ET.parse(sip_mets_path, parser)
        ...
        aip_parse = ET.parse(aip_mets_path, parser)
        ...
        str = ET.tostring(aip_root, encoding='UTF-8', pretty_print=True, xml_declaration=True)
...
class DeliveryValidation(object):
    ...
    def getFileElements(self, deliveryDir, delivery_xml_file, schema_file):
        ...
        try:
            ...
            # Parse the XSD file
            parsed_sfile = ET.parse(schema_file)
        ...
        except ET.XMLSyntaxError as why:
    ...
    def validate_delivery(self, deliveryDir, checksum_file, schema_file, package_file):
        ...
        @param      package_file:  Path to package file (e.g. TAR).
...
class SIPGenerator():
    ...
    def createDeliveryMets(self, input_archive, output_mets):
        ...
        mets_hdr.append(self.createAgent("ARCHIVIST", "ORGANIZATION", "", "E-ARK", "Demonstrator"))
        mets_hdr.append(self.createAgent("CREATOR", "ORGANIZATION", "", "E-ARK", "Demonstrator"))
        mets_hdr.append(self.createAgent("CREATOR", "OTHER", "SOFTWARE", "E-ARK Web SIP Creator", "VERSION=1.0"))
        mets_hdr.append(self.createAgent("PRESERVATION", "ORGANIZATION", "", "E-ARK", "Demonstrator"))
        ...
        xml_content = ET.tostring(root, encoding='UTF-8', pretty_print=True, xml_declaration=True)
        with open(output_mets, 'wb') as output_file:
